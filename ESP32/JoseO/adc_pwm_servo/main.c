...
static void set_dutyCicle(uint32_t dutyCicle_inUs){

	if (mcpwm_set_duty_in_us(MCPWM_UNIT, MCPWM_TIMER, MCPWM_OPERATOR, dutyCicle_inUs) == ESP_OK)
		printf("timpo en microsegundos: %d \r\n",dutyCicle_inUs);
	else printf("FAIL IN TIME..\r\n");
}
...
void mainTask_config(){
	uint32_t dutyCicle_inUs = 0, potData = 0, potData_inAngles = 0;
	...
	while(TRUE){
		...
		potData_inAngles = (potData*MAX_ANGLE)/1023;
		printf("potdata_in Angles: %d\r\n",potData_inAngles);
		dutyCicle_inUs = ((MIN_DEGREE) + ((potData_inAngles*MIN_DEGREE)/(MAX_ANGLE)));
		set_dutyCicle(dutyCicle_inUs);
		delay_ms(50);
	}
}
void get_potData(){
	uint32_t potData = 0; //datos de 0-1023 en el puerto 35.
	xSemaphoreTake(runPotData,portMAX_DELAY);
...
void app_main(){
	...
	potDataQueue = xQueueCreate(queueTam, sizeof(uint32_t));
	printf("ADC y PWM... \n");
