...
static void receive_callback (const uint8_t *mac_addr, const uint8_t *data, int data_len){
	...
	packet* msg = (packet*) data;
	char buff[20] = {0};
	sprintf(buff, MACSTR, MAC2STR(mac_addr));
	printf("mensaje recibido desde %s mesaje %s con id=%d\r\n", buff, msg->msg, msg->id);
}
...
void main_task(){
	...
	unsigned char* ptrPacket = NULL;
	ptrPacket = malloc(sizeof(packet));
	packet* datos = (packet*)ptrPacket;

	memcpy(datos->msg, "hola", sizeof("hola"));
	datos->id = 5;
	...
	while(1){
		if(esp_now_send(mac_broadcast, ptrPacket, sizeof(packet)) == ESP_OK){
